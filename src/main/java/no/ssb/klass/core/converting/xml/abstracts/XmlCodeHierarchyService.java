package no.ssb.klass.core.converting.xml.abstracts;
...
public abstract class XmlCodeHierarchyService<T, U> extends AbstractXmlService<T> {
    ...
    protected abstract ClassificationItem createClassificationItem(U xmlItem, T owner) throws ImportException;
    ...
    private ClassificationItem createIfMissingItem(XmlCodeHierarchy xmlItem, StatisticalClassification classification,
        ...
        ClassificationItem variantItem = createClassificationItem((U) xmlItem, (T) classification);
        ...
        if (StringUtils.isEmpty(parentCode)) {
            ...
            if (classification.hasClassificationItem(parentCode)) {
                // parent exists in version (OK)
                ClassificationItem parent = classification.findItem(parentCode);
                itemMap.put(variantItem, parent);
                return variantItem;
            } else {
                // parent should exist in import file (OK) (throws ImportException if unable to create parent)
