...
public interface SearchWordsRepository extends JpaRepository<SearchWords, Long> {
    @Query(value = "select count(distinct sw.searchString) from SearchWords sw "
    ...
    @Query(value = "select new no.ssb.klass.core.service.dto.AccessCounterDtoImpl(sw.searchString, count(sw.searchString)) "
            + "from SearchWords sw "
    ...
    @Query(value = "select new no.ssb.klass.core.service.dto.AccessCounterDtoImpl(sw.searchString, count(sw.searchString)) "
            ...
            + "order by count(sw.searchString) desc, sw.searchString")
    Page<StatisticalEntity> getSearchWords(@Param("fromDate") Date fromDate, @Param("toDate") Date toDate, Pageable p);
