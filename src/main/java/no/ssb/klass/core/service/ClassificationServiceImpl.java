...
public class ClassificationServiceImpl implements ClassificationService {
    ...
    public ClassificationSeries saveAndIndexClassification(ClassificationSeries classification) {
        classification = classificationRepository.save(classification);
        searchService.indexSynch(classification);
        return classification;
    }
    ...
    public ClassificationVersion saveNotIndexVersion(ClassificationVersion version) {
        deleteAnyDeletedReferencesToClassificationItems(version.getDeletedClassificationItems());
     ...
     */
    private void deleteAnyDeletedReferencesToClassificationItems(List<ClassificationItem> deletedClassificationItems) {
        for (ClassificationItem deletedClassificationItem : deletedClassificationItems) {
            if (deletedClassificationItem.getId() == null) {
                continue;
            }
            List<ReferencingClassificationItem> softDeletedItems = findClassificationItemsReferencing(
                    deletedClassificationItem, true);
            softDeletedItems.forEach(item -> item.getLevel().getStatisticalClassification().deleteClassificationItem(
                    item));
            Set<CorrespondenceMap> softDeletedCorrespondenceMaps = findCorrespondenceMapsReferencing(
                    deletedClassificationItem, true);
            softDeletedCorrespondenceMaps.forEach(map -> map.getCorrespondenceTable().removeCorrespondenceMap(map));
        }
    ...
    private Set<CorrespondenceMap> findCorrespondenceMapsReferencing(ClassificationItem classificationItem,
            boolean deleted) {
        List<CorrespondenceMap> sourceRefs = correspondenceMapRepository.findBySource(classificationItem);
        List<CorrespondenceMap> targetRefs = correspondenceMapRepository.findByTarget(classificationItem);
        Set<CorrespondenceMap> correspondenceMapsReferencing = new HashSet<>();
        for (List<CorrespondenceMap> refs : Arrays.asList(sourceRefs, targetRefs)) {
            correspondenceMapsReferencing.addAll(refs.stream().filter(map -> map.isDeleted() == deleted).collect(
                    toList()));
        }
        return correspondenceMapsReferencing;
    }
    ...
    private List<ReferencingClassificationItem> findClassificationItemsReferencing(
        ...
        return referencingClassificationItemRepository.findByReference(classificationItem).stream().filter(item -> item
                .isDeleted() == deleted).collect(toList());
    }
    ...
    @Override
    public List<Correspondence> findCorrespondences(Long sourceClassificationId, Long targetClassificationId,
    ...
    @Override
    public List<Code> findVariantClassificationCodes(Long id, String variantName, Language language,
            DateRange dateRange) {
    ...
    @Override
    public List<Code> findClassificationCodes(Long id, DateRange dateRange, Language language) {
        checkNotNull(dateRange);
        checkNotNull(language);
        ClassificationSeries classification = getClassificationSeries(id);
        return ClassificationServiceHelper.findClassificationCodes(classification, dateRange, language);
    }
