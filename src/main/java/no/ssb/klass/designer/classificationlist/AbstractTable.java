...
public abstract class AbstractTable extends CustomComponent {
    ...
    protected TextField createFilterBox(Table classificationTable) {
        TextField filter = new TextField();
        filter.setTextChangeEventMode(TextChangeEventMode.TIMEOUT);
        filter.setTextChangeTimeout(500);
        filter.setInputPrompt("Filtrer kodeverk");
        filter.addStyleName(ValoTheme.TEXTFIELD_TINY);
        filter.setWidth("100%");
        filter.addTextChangeListener(new SimpleTextChangeListener(classificationTable, AbstractPropertyContainer.NAME));
        return filter;
    }
