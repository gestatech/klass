...
import no.ssb.klass.core.converting.xml.FullVersionExportService;
...
import no.ssb.klass.designer.editing.VersionMainView;
...
public class VersionTable extends AbstractTable {
    ...
    public void init(ClassificationTable classificationTable, VariantTable variantTable) {
        ...
        rootLayout.addComponents(createHeader("Versjoner", exportVersionButton, addVersionButton), table);
    ...
    private static class ClassificationVersionClickListener implements ItemClickListener {
        ...
        private List<ClassificationEntityOperations> getListOfVariantsAndCorrespondenceTables(
            ...
            entities.addAll(classificationFacade.findCorrespondenceTablesWithSource(version));
            entities.addAll(classificationFacade.findCorrespondenceTablesWithTarget(version));
            entities.addAll(reloadToAvoidLazyInitializationException(version).getClassificationVariants());
            return entities;
        }
    ...
    private static class VersionContainer extends AbstractPropertyContainer<ClassificationVersion> {
        ...
        protected void defineProperties() {
            addProperty(AbstractPropertyContainer.NAME, String.class, null);
            addProperty(AbstractPropertyContainer.OPEN, Button.class, null);
            addProperty(AbstractPropertyContainer.DELETE, Button.class, null);
            addProperty(AbstractPropertyContainer.ENTITY, ClassificationVersion.class, null);
            hideProperty(AbstractPropertyContainer.ENTITY);
        }

        @SuppressWarnings("unchecked")
        @Override
        protected void populateProperties(Item item, ClassificationVersion version) {
            item.getItemProperty(AbstractPropertyContainer.NAME).setValue(version.getNameInPrimaryLanguage());
            item.getItemProperty(AbstractPropertyContainer.OPEN).setValue(createViewButton(version));
            item.getItemProperty(AbstractPropertyContainer.DELETE).setValue(createDeleteButton(version));
            item.getItemProperty(AbstractPropertyContainer.ENTITY).setValue(version);
        }
        ...
        private Button createViewButton(ClassificationVersion version) {
            ...
            openButton.addClickListener(event -> openVersion(event));
            return openButton;
        }
