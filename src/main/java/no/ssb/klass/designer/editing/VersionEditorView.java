...
public class VersionEditorView extends VersionEditorDesign implements HasEditingState {
    ...
    public void init(ClassificationVersion classificationVersion) {
        ...
        resetForm();
        Date from = TimeUtil.toDate(classificationVersion.getDateRange().getFrom());
        Date to = TimeUtil.toDate(VaadinUtil.convertToInclusive(classificationVersion.getDateRange().getTo()));
    ...
    public boolean updateVersion(ClassificationVersion classificationVersion,
        ...
        DateRange newRange = DateRange.create(TimeUtil.toLocalDate(fromDate.getValue()), VaadinUtil.convertToExclusive(
                TimeUtil.toLocalDate(toDate.getValue())));
        String overlapName = checkInterval(classificationVersion, newRange, classificationFacade);
        if (overlapName != null) {
            dateOverlapError(overlapName);
            return false;
        }
    ...
    private boolean checkForm() {
        removeErrorMarkings();
        return verifyDateInput() && verifyTextInput(introduction, "Beskrivelse må være fylt ut");
    }

    private String checkInterval(ClassificationVersion classificationVersion, DateRange range,
            ClassificationFacade classificationFacade) {
        for (ClassificationVersion version : reloadToAvoidLazyInitializationException(classificationVersion
                .getOwnerClassification(), classificationFacade).getClassificationVersions()) {
            if (!classificationVersion.equals(version) && range.overlaps(version.getDateRange())) {
