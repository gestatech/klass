...
public abstract class EditableCodeTable extends BaseCodeTable {
    ...
    private List<CodeEditor> getOpenCodeEditors() {
        return getContainer().getItemIds().stream().map(itemId -> getCodeEditorOrNull(itemId)).filter(
                codeEditor -> codeEditor != null).collect(toList());
    }
    ...
    public void levelDeleted(LevelDeletedEvent event) {
        version.getAllClassificationItems().forEach(classificationItem -> updateChildrenAllowed(classificationItem));
    ...
    public void closeHierarchy(CloseHierarchyEvent event) {
        for (ClassificationItem classificationItem : version.getAllClassificationItems()) {
