...
public class PrimaryCodeTable extends EditableCodeTable {
    ...
    public void init(EventBus eventbus, StatisticalClassification version, Language language,
        ...
        super.init(eventbus, version, language);
        if (isReadOnly()) {
    ...
    protected CodeEditor createRowSelectedCodeEditor(ClassificationItem classificationItem) {
        ...
        return new UpdateCodeEditor(eventbus, version, classificationItem, language);
    }

    @Override
    protected void updateChildrenAllowed(ClassificationItem classificationItem) {
        boolean isLastLevel = version.isLastLevel(classificationItem.getLevel());
        ...
        if (isLastLevel) {
            ...
            addNewCodeEditor(classificationItem, version.getNextLevel(classificationItem.getLevel()).get());
        }
    }
    ...
    protected void populateContainer() {
        ...
        if (!version.getLevels().isEmpty()) {
            addNewCodeEditor(null, version.getFirstLevel().get());
        }
    }

    private List<ClassificationItem> getNotLeafClassificationItems() {
        List<ClassificationItem> notLeafItems = new ArrayList<>();
        Optional<Level> level = version.getFirstLevel();
        while (level.isPresent() && !version.isLastLevel(level.get())) {
            notLeafItems.addAll(level.get().getClassificationItems());
            level = version.getNextLevel(level.get());
        }
        return notLeafItems;
    }
    ...
    protected void moveNewCodeEditorLast(ClassificationItem parent) {
        ...
        addNewCodeEditor(parent, version.getNextLevel(parent.getLevel()).get());
    }
    ...
    protected void addNewCodeEditor(ClassificationItem parent, Level level) {
        ...
        NewCodeEditor codeEditor = new NewCodeEditor(eventbus, version, parent, level, language);
    ...
    public void checkFirstLevelCreated(LevelCreatedEvent event) {
        if (version.isFirstLevel(event.getCreatedLevel())) {
