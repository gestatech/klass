package no.ssb.klass.designer.editing.codetables.containers;
 ...
 *
 *         Customized copy of vaadins HierarchicalContainer. Vaadin has made it quite difficult to override default
 *         behaivior so the easiest to get around this is to simply copy the class.
 *
 *         Noteworthy changes:
 *         <p>
 *         getUnfilteredItem // made public <br/>
 *         getUnfilteredParent // new method <br/>
 *
 *         setParent // changed to use getUnfilteredItem to check if our container contains provided item.<br/>
 *         We need to do this or else setParent will fail and our drag/drop item will become a root element )
 *         </p>
 *
 */
public class CustomHierarchicalContainer extends IndexedContainer implements
        Container.Hierarchical {

    @Override
    public Item getUnfilteredItem(Object itemId) {
        return super.getUnfilteredItem(itemId);
    }

    public Object getUnfilteredParent(Object itemId) {
    ...
    /*
     * Can the specified Item have any children? Don't add a JavaDoc comment here, we use the default documentation from
    ...
    /*
     * Gets the IDs of the children of the specified Item. Don't add a JavaDoc comment here, we use the default
    ...
    /*
     * Gets the ID of the parent of the specified Item. Don't add a JavaDoc comment here, we use the default
     * documentation from implemented interface.
     */
    @Override
    ...
    /*
     * Is the Item corresponding to the given ID a leaf node? Don't add a JavaDoc comment here, we use the default
     * documentation from implemented interface.
     */
    @Override
    ...
    /*
     * Is the Item corresponding to the given ID a root node? Don't add a JavaDoc comment here, we use the default
     * documentation from implemented interface.
     */
    @Override
    ...
    /*
     * Gets the IDs of the root elements in the container. Don't add a JavaDoc comment here, we use the default
     * documentation from implemented interface.
     */
    @Override
    ...
    /**
     * <p>
     * Sets the given Item's capability to have children. If the Item identified with the itemId already has children
     * and the areChildrenAllowed is false this method fails and <code>false</code> is returned; the children must be
     * first explicitly removed with {@link #setParent(Object itemId, Object newParentId)} or
     * {@link com.vaadin.data.Container#removeItem(Object itemId)}.
     * </p>
     *
     * @param itemId
     *            the ID of the Item in the container whose child capability is to be set.
    ...
    public boolean setChildrenAllowed(Object itemId, boolean childrenAllowed) {
        ...
        if (getUnfilteredItem(itemId) == null) {
            return false;
        }
     ...
     * <code>canHaveChildren(newParentId) == true</code>). It is also possible to detach a node from the hierarchy (and
     * thus make it root) by setting the parent <code>null</code>.
     * </p>
     *
     * @param itemId
     *            the ID of the item to be set as the child of the Item identified with newParentId.
     * @param newParentId
     *            the ID of the Item that's to be the new parent of the Item identified with itemId.
     * @return <code>true</code> if the operation succeeded, <code>false</code> if not
     */
    @Override
    ...
    /**
     * Moves a node (an Item) in the container immediately after a sibling node. The two nodes must have the same parent
     * in the container.
     *
     * @param itemId
    ...
    private void enableAndFireContentsChangeEvents() {
        if (contentChangedEventsDisabledCount <= 0) {
            getLogger()
                    .log(Level.WARNING,
                            "Mismatched calls to disable and enable contents change events in CustomHierarchicalContainer");
    ...
    /**
     * Used to control how filtering works. @see {@link #setIncludeParentsWhenFiltering(boolean)} for more information.
     *
     * @return true if all parents for items that match the filter are included when filtering, false if only the
     *         matching items are included
     */
    ...
    /**
     * Controls how the filtering of the container works. Set this to true to make filtering include parents for all
     * matched items in addition to the items themselves. Setting this to false causes the filtering to only include the
     * matching items and make items with excluded parents into root items.
     *
    ...
    /**
     * Recursively adds all items in the includedItems list to the filteredChildren map in the same order as they are in
     * the children map. Starts from parentItemId and recurses down as long as child items that should be included are
     * found.
     *
     * @param parentItemId
     *            The item id to start recurse from. Not added to a filteredChildren list
     * @param includedItems
     *            Set containing the item ids for the items that should be included in the filteredChildren map
     */
    ...
    /**
     * Scans the itemId and all its children for which items should be included when filtering. All items which passes
     * the filters are included. Additionally all items that have a child node that should be included are also
     * themselves included.
     *
     * @param itemId
     * @param includedItems
     ...
     * (non-Javadoc)
     *
     * @see com.vaadin.data.util.IndexedContainer#passesFilters(java.lang.Object)
     */
    @Override
    ...
    private static Logger getLogger() {
        return Logger.getLogger(CustomHierarchicalContainer.class.getName());
    }
}