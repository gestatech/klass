...
import no.ssb.klass.designer.editing.CreateVersionView;
...
public class NewVersionWindow extends Window {
            ...
            if (checkValidityPeriod(fromMonth, fromYear, toMonth, toYear)) {
                ...
                if (!copyVersionChoice.getValue()) {
                    VaadinUtil.navigateTo(CreateVersionView.NAME, ImmutableMap.of(
                            CreateVersionView.PARAM_CLASSIFICATION_ID,
                            classificationId, CreateVersionView.VALID_FROM_KEY, validFrom.toString(),
                            CreateVersionView.VALID_TO_KEY, validTo != null ? validTo.toString() : "null"));
                } else {
    ...
    private boolean checkValidityPeriod(TextField fromMonth, TextField fromYear, TextField toMonth, TextField toYear) {
        ...
        try {
            validFrom = LocalDate.of(checkAndGetValue(MANDATORY, "Fra år", fromYear, null), checkAndGetValue(OPTIONAL,
                    "Fra måned", fromMonth, DEFAULT_MONTH), DEFAULT_DATE);
